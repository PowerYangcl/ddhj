package cn.com.ddhj.service.apporderpay.alipay;
...
import cn.com.ddhj.service.apporderpay.alipay.config.AlipayMoveConfig;
import cn.com.ddhj.service.apporderpay.sign.RSA;
 ...
 * 
 */
public class ApiAlipayMoveProcessService {
...
//
//			Map<String, String> unionQueryMap = new HashMap<String, String>();
...
//
//				// 通过订单号查询订单金额
//				List<MDataMap> orderinfoList = DbUp.upTable(OC_ORDERINFO).queryAll("", "", "", queryHcOrder);
//
...
//	} 
	 ...
	 * @return
	 */
	public String alipayMoveParameterNew(String payCode, TPayInfo payInfo) {
		Map<String, String> map = new HashMap<String, String>();
		String strParam = null;
		String value = null;
		String sOut = "";
		...
		//商品名称
		String productNameAll = PropHelper.getValue("PRODUCT_NAME");
		if (payCode != null && !"".equals(payCode)) {
			...
			if("PP".equals(payCode.substring(0, 2))){    
				// 通过订单号查询订单金额
				if(payInfo != null) {
					alipayMoveConfigMap = alipayMoveConfig.alipayConfig();  //获取支付相关配置信息
					
					if(alipayMoveConfigMap!=null && !"".equals(alipayMoveConfigMap) && alipayMoveConfigMap.size()>0) {
						map = objectToMapValues(payInfo.getPayMoney().toString(), payCode, "", productNameAll,alipayMoveConfigMap,moveMark);
					}
				}
			}
...
//	}
//
	private Map<String, String> objectToMapValues(String payMoney, String out_trade_no, String body, String subject,Map<String, String> alipayMoveConfigMap,String mark) {
		Map<String, String> map = new HashMap<String, String>();
		if("001".equals(mark)){   //移动支付
			map.put("service", PropHelper.getValue("serviceMove"));
			map.put("notify_url", PropHelper.getValue("alipay_serviceMove_response"));
		}else if("002".equals(mark)){   //网页支付
			map.put("service", PropHelper.getValue("serviceWeb"));
			map.put("notify_url", PropHelper.getValue("alipay_serviceWeb_response"));
		}
		...
		map.put("total_fee", payMoney); // 库里查的
		map.put("body", body);
		/*
		 *  生成要请求给支付宝的参数数组
		 */
...
//		
//		Date createDate = DateUtil.strToDateTime(createTime);
//		
