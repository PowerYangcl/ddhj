...
//	public static String seller_email = "zhifubao@carinalau.cn";
	...
	public Map<String, String> alipayWebConfig(String sellerCode) {
		...
		if (sellerCode != null && !"".equals(sellerCode)) {
			...
			if(HUIJIAYOU_SELLORCODE.equals(sellerCode)){
				map.put("notify_url", PropHelper.getValue("alipay_serviceFamilyWeb_response")); // 惠家有回调地址
			}
			
			map.put("return_url", PropHelper.getValue("return_url_web")); // 支付成功后跳转页
			map.put("return_url_pinhaohuo", PropHelper.getValue("return_url_web_pinhaohuo")); // 拼好货支付成功跳转页

			
			if (HUIJIAYOU_SELLORCODE.equals(sellerCode)) {
				map.put("partner", PropHelper.getValue("partner_Huijiayou"));
				map.put("seller_email",	PropHelper.getValue("seller_email_Huijiayou"));
				map.put("key", PropHelper.getValue("key_Huijiayou"));
				map.put("public_key", PropHelper.getValue("public_key"));
				
				//手机网页版   wap(此属性加上，为支付宝WAP支付)
				map.put("service", PropHelper.getValue("serviceMoveWeb"));
			} 
		}
		return map;
	}
