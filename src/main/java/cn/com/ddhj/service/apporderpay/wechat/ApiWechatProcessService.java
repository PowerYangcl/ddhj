...
import cn.com.ddhj.service.apporderpay.wechat.util.AccessTokenRequestHandler;
import cn.com.ddhj.service.apporderpay.wechat.util.ClientRequestHandler;
import cn.com.ddhj.service.apporderpay.wechat.util.PackageRequestHandler;
import cn.com.ddhj.service.apporderpay.wechat.util.PrepayIdRequestHandler;
/**
 * 微信支付
 * @author wz
 *
...
//	}
	 ...
	 *
	 */
	public MDataMap wechatMovePaymentNew(String orderPayCode, TPayInfo payInfo, String ip, BaseResult rootResult)
			throws UnsupportedEncodingException {

		// 接收财付通通知的URL
		String notify_url = PropHelper.getValue("NOTIFY_URL_WEB");
		...
		PrepayIdRequestHandler prepayReqHandler = new PrepayIdRequestHandler();// 获取prepayid的请求类
		ClientRequestHandler clientHandler = new ClientRequestHandler();// 返回客户端支付参数的请求类
		packageReqHandler.setKey(PropHelper.getValue("PARTNER_KEY"));
...
//						&& orderMap.size() > 0) {
					 ...
					 */
					//String bodyString = URLEncoder.encode(String.valueOf(orderMap.get("product_name")).replaceAll(" ", ""));
					packageReqHandler.setParameter("body", PropHelper.getValue("PRODUCT_NAME")); // 商品描述
					packageReqHandler.setParameter("notify_url", notify_url); // 接收财付通通知的URL
					packageReqHandler.setParameter("partner",PropHelper.getValue("PARTNER")); // 商户号
					packageReqHandler.setParameter("out_trade_no", orderPayCode); // 商家订单号
					//String.valueOf(((BigDecimal)orderMap.get("due_money")).setScale(0,BigDecimal.ROUND_HALF_UP))+"00"
					packageReqHandler.setParameter("total_fee", String.valueOf((((BigDecimal)payInfo.getPayMoney()).multiply(new BigDecimal(100))).setScale(2,BigDecimal.ROUND_HALF_UP))); // 商品金额,以分为单位String.valueOf(orderMap.get("due_money"))
					packageReqHandler.setParameter("spbill_create_ip", ip); // 订单生成的机器IP，指用户浏览器端IP
					packageReqHandler.setParameter("fee_type", "1"); // 币种，1人民币// 66
					packageReqHandler.setParameter("input_charset", "UTF-8"); // 字符编码GBK
					 ...
					 * 设置获取prepayid支付参数
					 */
					prepayReqHandler.setParameter("appid", PropHelper.getValue("APP_ID"));    
					prepayReqHandler.setParameter("appkey",PropHelper.getValue("APP_KEY")); // 商户密钥// 缺少
					prepayReqHandler.setParameter("noncestr", noncestr);
					prepayReqHandler.setParameter("package", packageValue);
					prepayReqHandler.setParameter("timestamp", timestamp);
					prepayReqHandler.setParameter("traceid", traceid);
					...
					if (null != prepayid && !"".equals(prepayid)) {
						// 输出参数列表
						clientHandler.setParameter("appid", PropHelper.getValue("APP_ID"));  //商家在微信开放平台申请的应用id
						clientHandler.setParameter("appkey",PropHelper.getValue("APP_KEY"));
						clientHandler.setParameter("noncestr", noncestr);     /** 随机串，防重发 */
						//clientHandler.setParameter("package", "Sign=" + packageValue);
						clientHandler.setParameter("package", "Sign=WXPay");
						clientHandler.setParameter("partnerid",PropHelper.getValue("PARTNER"));
						clientHandler.setParameter("prepayid", prepayid);
						clientHandler.setParameter("timestamp", timestamp);    /** 时间戳，防重发 */
						// 生成签名
						sign = clientHandler.createSHA1Sign();
