...
public class ApiController extends BaseClass {
	...
	public JSONObject api(BaseAPI api, HttpSession session, HttpServletRequest request) {
		...
		// 根据楼盘编码查询楼盘环境报告
			...
			String lpCode = obj.getString("lpCode");
			JSONObject result_ = estateEnvService.apiEstateList(position, city, page, count, radius, lpCode, application);
			long end = System.currentTimeMillis();
		 ...
		 else if ("2068".equals(api.getApiTarget())) { // 手动刷楼盘得分  http://localhost:8080/ddhj/api.htm?apiTarget=2068&apiInput={"code":"LP161004111543"}
				long start = System.currentTimeMillis();
//				estateEnvService.ressssyncEstateLoad(obj.getString("city")); 
				String code = obj.getString("code");
				estateEnvService.jobForRessssyncEstateLoadByCode(code);
